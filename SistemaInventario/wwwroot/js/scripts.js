...
!function (NioApp, $) {
    ...
    NioApp.ModeSwitch = function () {
        ...
        var expira = new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 365).toGMTString();
        if (system.getCookie('invetariostema') == 'dark') {
            $body.addClass('dark-mode')
            document.cookie = 'invetariostema=dark; expires=' + expira +'; path=/';            
        }
        else {
            $body.removeClass('dark-mode')
            document.cookie = 'invetariostema=white; expires=' + expira +'; path=/';
        }
        ...
        toggle.on('click', function (e) {
            ...
            expira = new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 365).toGMTString();
            if ($body.hasClass('dark-mode'))
                document.cookie = 'invetariostema=dark; expires=' + expira +'; path=/';
            else
                document.cookie = 'invetariostema=white; expires=' + expira +'; path=/';
